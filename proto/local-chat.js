...
async function send(text,kind,retry=false){
  ...
  function consume(line){
    ...
    if(event.type==='meta'){if(typeof refreshDailyQuota==='function')refreshDailyQuota({force:true});message.sources=event.sources;message.source=event.source;message.retrieval=event.retrieval;message.contextTrimmed=event.trimmed;}
  ...
  try{
    ...
    if(typeof refreshDailyQuota==='function')refreshDailyQuota({force:true});
  }
}
